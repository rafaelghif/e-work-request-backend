...
const generateTicketNumber = (data) => {
	...
	if (currentYear === ticketYear && currentMonth === ticketMonth) {
		...
		const newTicketNumberSequence = (ticketNumberSequence + 1)
			.toString()
			.padStart(3, "0");
...
export const getWorkRequest = async (req, res) => {
	try {
		...
		const responseSection = await connectionDatabase.query(
			`SELECT DISTINCT level FROM sections WHERE DepartmentId = '${department.id}' ORDER BY level ASC`,
		...
		const accept = [
			{
				level: [0, 1, 2],
				userLevel: 0,
				status: ["Approve", "Progress", "Pending"],
			},
			{
				level: [0, 1, 2],
				userLevel: 1,
		...
		const userAccept = accept.find(
			({ level, userLevel }) =>
				level.every((value, index) => value === userLevelDb[index]) &&
				userLevel === parseInt(section.level),
		);
...
export const getTicketRequest = async (req, res) => {
	try {
		...
		const { department, section } = req.decoded;
		const responseSection = await connectionDatabase.query(
			`SELECT DISTINCT level FROM sections WHERE DepartmentId = '${department.id}' ORDER BY level ASC`,
			{ type: QueryTypes.SELECT },
		);
...
export const getWorkRequestReceive = async (req, res) => {
	try {
		...
		const userAccept = accept.find(
			({ level, acceptLevel }) =>
				level.every((value, index) => value === userLevelDb[index]) &&
				acceptLevel.includes(parseInt(section.level)),
		);
...
export const getWorkRequestYear = async (req, res) => {
	try {
		const response = await connectionDatabase.query(
			"SELECT DISTINCT YEAR(createdAt) as ticketYear FROM tickets ORDER BY ticketYear ASC",
			{ type: QueryTypes.SELECT },
		);
...
export const getWorkRequestMonth = async (req, res) => {
	try {
		const response = await connectionDatabase.query(
			"SELECT DISTINCT MONTH(createdAt) as ticketMonth FROM tickets ORDER BY ticketMonth ASC",
			{ type: QueryTypes.SELECT },
		);
...
export const getWorkRequestType = async (req, res) => {
	try {
		const response = await connectionDatabase.query(
			"SELECT DISTINCT id,format FROM registrationnumbers WHERE id IN (SELECT DISTINCT RegistrationNumberId FROM tickets) ORDER BY format ASC",
			{ type: QueryTypes.SELECT },
		);
...
export const getWorkRequestDepartment = async (req, res) => {
	try {
		const response = await connectionDatabase.query(
			"SELECT DISTINCT id,name FROM departments WHERE id IN (SELECT DISTINCT RequesterDepartmentId FROM tickets) ORDER BY name ASC",
			{ type: QueryTypes.SELECT },
		);
				...
				[Op.and]: [
					...
					connectionDatabase.where(
						connectionDatabase.fn(
							"YEAR",
							connectionDatabase.col("Ticket.createdAt"),
						),
						yearFilter,
					),
				],
...
export const getWorkRequestIssued = async (req, res) => {
	try {
		...
		return res.status(200).json({
			message: "Success Fetch Work Requests Issued!",
			data: response,
		});
	} catch (err) {
		errorLogging(err.toString());
...
export const createWorkRequest = async (req, res) => {
	...
	try {
		...
		const { id: UserDepartmentId } = req.decoded.department;
		...
		const ticketNumber =
			responseWorkRequest === null
				? "WR-2303-001"
				: generateTicketNumber(responseWorkRequest);

		const responseRegistrationNumber =
			await models.RegistrationNumber.findByPk(RegistrationNumberId);
		const { format, year, lastNumber } = responseRegistrationNumber;
		let newLastNumber = parseInt(lastNumber) + 1;

		await models.RegistrationNumber.update(
			{ lastNumber: newLastNumber },
			{ where: { id: RegistrationNumberId }, transaction },
		);
		const currentDate = new Date();
		const currentYear = currentDate.getFullYear();
		const stringYear = currentYear
			.toString()
			.substring(2, 4)
			.padStart(2, "0");
		let newWorkNumber = `${format}-${stringYear}-${newLastNumber
			.toString()
			.padStart(3, "0")}`;

		if (year !== currentYear) {
			await models.RegistrationNumber.update(
				{ year: currentYear, lastNumber: 1 },
				{ where: { id: RegistrationNumberId }, transaction },
			);
			newWorkNumber = `${format}-${stringYear}-${"1"
				.toString()
				.padStart(3, "0")}`;
		}
		...
		const assignDepartments = AssigneeDepartmentIds.map((res) => ({
			AssigneeDepartmentId: res,
			ApproverDepartmentId: UserDepartmentId,
			createdBy: badgeId,
			updatedBy: badgeId,
			TicketId: response.id,
		}));
		await models.TicketAssignee.bulkCreate(assignDepartments, {
			transaction,
		});
...
export const picActionTicket = async (req, res) => {
	...
	try {
		...
		if (ticketStatus === "Progress") {
			...
			const ticketAssignCount = await models.TicketAssignee.count({
				where: { TicketId: id, inActive: false },
			});
...
export const sendMailCreateWorkRequest = async (
	email,
	badgeId,
	name,
	department,
...
export const updateWorkRequest = async (req, res) => {
	...
	try {
		...
		const { id, ticketNumber, description, jigToolNo, qty, expectDueDate } =
			req.body;
		const { id: UserId, badgeId, name } = req.decoded.user;
